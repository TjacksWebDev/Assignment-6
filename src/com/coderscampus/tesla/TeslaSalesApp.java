...
public class TeslaSalesApp {
	...
	private static void showReport(List<SalesData> carSalesData, String modelType) {
		...
		String totalYearlySales = groupedByYearData.entrySet().stream().map(
				d -> d.getKey() + " -> " + d.getValue().stream().collect(Collectors.summingInt(SalesData::getSales)))
		...
		System.out.println("The best month for " + modelType + " was: "
				+ maxSalesData.orElse(new SalesData("Jan-70", "0")).getDate());
		System.out.println("The worst month for " + modelType + " was: "
				+ minSalesData.orElse(new SalesData("Jan-70", "0")).getDate() + "\n");
		System.out.println("=============================");
